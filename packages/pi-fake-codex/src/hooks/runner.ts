...
// Source: packages/pi-maestro-flow/src/hooks/runner.ts
// Adapted: trust/review toggles removed (pi-fake-codex has no trust store);
// hookHandlerId/hookHandlerIdentity imports dropped with them.
...
export function getMatchingCommandHooks(
  ...
  for (const group of config.hooks[eventName] ?? []) {
    ...
    for (const handler of group.hooks) {
      if (groupMatches && handler.type === "command" && !handler.async) handlers.push(handler);
    }
  }
  return handlers;
}
